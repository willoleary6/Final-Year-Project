...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        break_out = False
        cap = cv2.VideoCapture('rtsp://'+Config.CAMERA_USERNAME+':'+Config.CAMERA_PASSWORD+'@'+Config.CAMERA_IP_ADDRESS+':'+Config.CAMERA_PORT+'/videoMain');
        end_of_video = False
        while True:
            ...
            if frame_number % Config.FRAME_DELIMITER_FOR_TENSORFLOW == 0:
                ...
                detector_object.new_detection('livestream', timestamp, len(objects))
